...
from tkinter import Tk, Button, Label, Listbox, StringVar, font

def traducir():
    entrada = lista_idiomas.curselection()
    if entrada != ():
        idioma = lista_idiomas.get(entrada)
        if idioma ==label( font= "Garamond", show="Inglés", show= "Hi there")
...
palabra_original = Label(text="Hola", font=("Garamond", 12, "bold italic"))
boton = Button(text="-->", command=traducir)
palabra_traducida = Label(text="¿?", font=("Garamond", 12, "bold italic"))
